...
runFunction(function()
	...
	local noclickdelay = {Enabled = true}
...
local AnticheatDisabler = COB("Combat", {
    Name = "InfernalKill",
    Function = function(callback) 
        if callback then
			toggle = true
    ...
    Default = false,
    HoverText = "uses infernal saber to kill"
})
end)
runFunction(function()
			...
			if callback then 
				table.insert(AutoToxic.Connections, vapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
					if AutoToxicBedDestroyed.Enabled and bedTable.brokenBedTeam.id == lplr:GetAttribute("Team") then
						local custommsg = #AutoToxicPhrases6.ObjectList > 0 and AutoToxicPhrases6.ObjectList[math.random(1, #AutoToxicPhrases6.ObjectList)] or "no bed no game <name> | acronsiware on top"
						if custommsg then
					...
					elseif AutoToxicBedBreak.Enabled and bedTable.player.UserId == lplr.UserId then
						local custommsg = #AutoToxicPhrases7.ObjectList > 0 and AutoToxicPhrases7.ObjectList[math.random(1, #AutoToxicPhrases7.ObjectList)] or "looks like your bed was nuked <teamname> | Acronisware on top"
						if custommsg then
					...
					if deathTable.finalKill then
						...
						if killed == lplr then 
							if (not leavesaid) and killer ~= lplr and AutoToxicDeath.Enabled then
								leavesaid = true
								local custommsg = #AutoToxicPhrases3.ObjectList > 0 and AutoToxicPhrases3.ObjectList[math.random(1, #AutoToxicPhrases3.ObjectList)] or "Devs couldve done better <name>| Acronisware on top"
								if custommsg then
							...
							if killer == lplr and AutoToxicFinalKill.Enabled then 
								local custommsg = #AutoToxicPhrases2.ObjectList > 0 and AutoToxicPhrases2.ObjectList[math.random(1, #AutoToxicPhrases2.ObjectList)] or "sucks for you, <name> | Acronisware on top"
								if custommsg == lastsaid then
									custommsg = #AutoToxicPhrases2.ObjectList > 0 and AutoToxicPhrases2.ObjectList[math.random(1, #AutoToxicPhrases2.ObjectList)] or "Acronisware didn't let that slide <name> | Acronisware on top"
								else
									lastsaid = custommsg
								end
								if custommsg then
				...
				table.insert(AutoToxic.Connections, vapeEvents.MatchEndEvent.Event:Connect(function(winstuff)
					...
					if myTeam and myTeam.id == winstuff.winningTeamId or lplr.Neutral then
						if AutoToxicGG.Enabled then
							replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("ggs match well done", "All")
							if shared.ggfunction then
								shared.ggfunction()
							end
						end
						if AutoToxicWin.Enabled then
							replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(#AutoToxicPhrases.ObjectList > 0 and AutoToxicPhrases.ObjectList[math.random(1, #AutoToxicPhrases.ObjectList)] or "Acronisware better then legits | Acronisware on top", "All")
						end
					end
				end))
				table.insert(AutoToxic.Connections, vapeEvents.LagbackEvent.Event:Connect(function(plr)
					if AutoToxicLagback.Enabled then
						...
						local msg = custommsg or "only legits get lackbacked "..(plr.DisplayName or plr.Name).." | Acronisware on top"
					...
					if AutoToxicRespond.Enabled then
						...
						if plr and plr ~= lplr and not alreadyreported[plr] then
							...
							if reportreason then 
								...
								local msg = custommsg or "I couldnt care less, you died in a lego game "..(plr.DisplayName or plr.Name).." | Acronisware on top"
								replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(msg, "All")
							end
						end
					end
